...
void objectRandom(int upperBG[], int lowerBG[]) {
    ...
    case lowerObject1:
        ...
        if ((lowerBG[15] == lowerObject1) || (lowerBG[14] == lowerObject1) ||
            (lowerBG[13] == lowerObject1) || (upperBG[15] == upperObject1)) {
            lowerBG[16] = empty;
        } else
...
void detect() {
    if ((dino_up_pos == 1 && upperBG[0] != 7) ||
        (dino_low_pos == 1 && lowerBG[0] != 7)) {
        lcd.clear();
        lcd.setCursor(1, 0);
        playing = false;
        scoring = false;
